...
function Calendar() {
  ...
  const [todaysEvents, setTodaysEvents] = useState([]); // State to store today's events
  ...
  useEffect(() => {
    fetchEvents();
    fetchTodaysEvents(); // Fetch today's events on component mount
  }, []);
  ...
  const confirmDeleteEvent = async () => {
    if (deleteEventId) {
      try {
        ...
        fetchEvents();
        fetchTodaysEvents(); // Refresh today's events after deletion
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      fetchEvents();
      fetchTodaysEvents(); // Refresh today's events after creation
